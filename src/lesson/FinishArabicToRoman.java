...
enum FinishArabicToRoman {
    ...
    XL("40"), XLI("41"), XLII("42"), XLIII("43"),
    XLIV("44"), XLV("45"), XLVI("46"), XLVII("47"),
    ...
    XCI("91"), XCII("92"), XCIII("93"), XCIV("94"),
    XCV("95"), XCVI("96"), XCVII("97"), XCVIII("98"),
    XCIX("99"), C("100");

    private final String arabicNumber;

    FinishArabicToRoman(String arabicNumber) {
        this.arabicNumber = arabicNumber;
    }


    String getArabicNumber() {
        return arabicNumber;
    }

    static FinishArabicToRoman getFinish(String arabicNumber) { // Перевод результата
        // из араб. системы счисления на рим.
        for (FinishArabicToRoman fin : values()) {
            if (fin.getArabicNumber().equals(arabicNumber)) {
                return fin;
            }
        }
        throw new IllegalArgumentException("Не найдено данное перечисление");
    }
